...
Page({
  ...
  onLoad: function (options) {
    ...
    var checkInDate = options.checkInDate ? options.checkInDate : app.Moment(new Date()).format('YYYY-MM-DD');
    var checkOutDate = options.checkOutDate ? options.checkOutDate : app.Moment(new Date()).add(1, 'day').format('YYYY-MM-DD');
    wx.getSystemInfo({
  ...
  createDateListData: function () {
    ...
    */
    ...
    for (var i = 0; i < this.data.maxMonth; i++) {
      var momentDate = app.Moment(now).add(this.data.maxMonth - (this.data.maxMonth - i), 'month').date;
  ...
  onPressDate: function (e) {
    ...
    if ((this.data.markcheckInDate && app.Moment(date).before(this.data.checkInDate) || this.data.checkInDate === date)) {
      this.setData({
    ...
    if (!this.data.markcheckInDate) {
      ...
      prevPage.setData({ checkInDate: this.data.checkInDate, checkOutDate: this.data.checkOutDate })//设置数据
      wx.navigateBack({
